# AI JOB FILTER — Architecture Decision Record v1.1

**Version:** 1.1 (FREE-FIRST MVP)  
**Previous:** v1.0  
**Date:** 2026-08-21  
...
| **Vision / Images** | google-genai (Gemini Flash) | 2.19+ | **Free tier** | Image → structured JSON |
...
**Verified fact (source: ai.google.dev/pricing):**
- Gemini 2.0 Flash: 15 RPM, **1,500 RPD**, 1,000,000 TPM
...
flowchart TD
    ...
    subgraph "IMAGE PATH"
        ...
        I_VISION["Gemini Flash Vision<br/>(free tier)"]
    ...
    I_DEDUP_IMG -->|"New image"| I_CAPTION
    I_CAPTION -->|"Has caption"| I_CAPTION_DETECT
    I_CAPTION_DETECT -->|"No job keywords"| T_DROP
    I_CAPTION_DETECT -->|"Keywords / short caption"| I_VISION
    I_CAPTION -->|"No caption (image-only)"| I_VISION
...
Image-only message (no caption)
  ...
  → Send to Gemini Flash Vision (free tier)
...
Caption + Image message
  → Caption has job keywords? 
    → YES: Download image → Gemini Vision extracts from image
           → Merge vision result with caption text
           → LLM extraction on combined text
    → NO (but caption is very short, <50 chars):
           → Treat as image-only path
    → NO (caption is substantial, no job keywords):
           → DROP (not a job)
```
...
│       │   ├── vision.py        # Gemini Flash image extraction
...
| `gemini_provider.py` | `VisionProvider` + `TextProvider` | `google-genai` (2.19+) | `VISION_MODEL=gemini-2.0-flash` |
...
### Job-Level Dedup (Post-Extraction)

After LLM extraction, compare against recent jobs (30 days):
```python
# Pseudocode
existing = repo.find_recent_jobs(days=30)
for job in existing:
    title_sim = fuzz.token_set_ratio(new_job.title, job.title)
    company_sim = fuzz.token_set_ratio(new_job.company or "", job.company or "")
    if title_sim >= 85 and company_sim >= 80:
        # Same job from different channel
...
| Vision Extraction | Gemini 2.0 Flash | Free (1,500 RPD) | **$0.00** |
...
| Gemini Flash requests | 1,500 RPD | ~20–50 images | 30–75x |
| Gemini Flash tokens | 1,000,000 TPM | ~20,000/day | 50x+ |
...
| Vision extraction | Gemini Flash free tier |
...
| **Image-only posts with low Vision accuracy** | 🟡 Medium | 🟢 Low | Gemini Flash excels at poster/flyer extraction; fallback to PENDING_VISION |
...
> [!IMPORTANT]
> All limits verified from official documentation as of August 2026. Free tiers may change — the provider abstraction allows migration.
...
### Gemini Free Tier (Verified: ai.google.dev/pricing)

| Model | RPM | RPD | TPM |
|:---|:---|:---|:---|
| Gemini 2.0 Flash | 15 | **1,500** | 1,000,000 |
| Gemini 2.0 Flash-Lite | 30 | 1,500 | 4,000,000 |
